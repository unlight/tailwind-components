...
export default async function ({ page }: ScraperArgs): Promise<CompomentLink[]> {
    await page.goto('https://devdojo.com/tailwindcss/components', { waitUntil: 'networkidle0' });

    while (true) {
        const isEnd = await page.evaluate(() => {
            const node = document
                .evaluate('//*[text()="Those are all the components we have for now."]', document)
                .iterateNext();
        ...
        await page.evaluate(() => {
            const node = document.evaluate('//*[text()="Load More"]', document).iterateNext();
